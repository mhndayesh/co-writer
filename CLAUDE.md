...
**Routing (split technical vs creative).** A user has a `mode` (`single`/`split`/`custom`) on `user_llm_settings` plus optional `llm_profiles` rows keyed by `role`. [`services/llm/roles.py`](apps/api/app/services/llm/roles.py) maps each `page` to a category: `flow.polish`/`flow.companion`/`story_check` → **creative**, `flow.extract`/`llm.test` → **technical**, unknown → technical. Resolution in `get_provider_for_page`:
- `single` → the default profile (flat columns on `user_llm_settings`) — original behavior, zero change.
- `split` → the `creative` or `technical` `llm_profiles` row by category, falling back to default.
- `custom` → `task:<page>` row → category row → default.

Embeddings resolve separately via `get_embedding_provider` (used by `rag_service` + `/rag/reindex`): the `embedding` profile if embed-capable, else default if embed-capable, else **always** local LM Studio. `EMBED_CAPABLE = (lmstudio, openai, gemini)` in `factory.py` — Anthropic and OpenRouter can't embed; never route embeddings to them. When adding a new AI `page`, add it to `PAGE_CATEGORY` in `roles.py` (and to `CUSTOM_TASKS` if it should appear in the Custom-mode UI). Config is managed via `GET/PUT /v1/llm/config`; `/settings` is a back-compat alias onto the default profile; `/status` returns one entry per active role.

Providers all implement `chat()` + `embed()` + `ping()`. `lmstudio.py`, `openai_provider.py`, `anthropic_provider.py` are first-class; `openrouter_provider.py` and `gemini_provider.py` **subclass `OpenAIProvider`** (both speak the OpenAI chat protocol) — OpenRouter overrides `embed()` to raise (chat-only router) and adds attribution headers; Gemini points at Google's OpenAI-compat base and keeps embeddings. To add another OpenAI-compatible provider, subclass `OpenAIProvider`, then register it in `factory.build_provider` + `EMBED_CAPABLE` (if it embeds) + the `ProviderName` Literal in `schemas.py` + frontend `PROVIDER_DEFAULTS`/`PROVIDER_LABELS`/`EMBED_INCAPABLE` in `ProviderForm.tsx`. **Hard-won quirks** baked in — don't undo:
...
- After pulling changes that touch the schema, run `alembic upgrade head` (or just `./run.sh`, which does it). A "no such column: …mode" / "no such table: llm_profiles" error at startup means migration `0002` hasn't been applied to your local `gink_dev.db`.
- Adding a provider that emits embeddings? Add it to `EMBED_CAPABLE` in `factory.py` AND `EMBED_INCAPABLE` (inverse) is **not** auto-derived on the frontend — update `EMBED_INCAPABLE` in `ProviderForm.tsx` too, or the embedding slot will offer a provider that can't embed.
